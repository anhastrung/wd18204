...
const DetailPage = () => {
    ...
    const { data: listProduct, isLoading: isLoadingList, refetch } = useHookQuery({ path: 'products', category: data?.category })
    ...
    const [listProduct1, setListProduct1] = useState<IProduct[]>([])
    useEffect(() => {
        if (listProduct) {
            setListProduct1(listProduct.slice(0, limit))
        }
    }, [listProduct, limit])
    useEffect(() => {
        if (data && data.category) refetch()
    }, [data, refetch])
    useEffect(() => {
        setImage(0)
    }, [setImage, id])
    const { user } = useContext(UserContext)
    const { data: listCart, refetch: refetchCart } = useCartQuery(user && user.id)
    ...
    const handleAddToCart = (product: IProduct) => {
        if (timer > 0) {
            ...
            if (!cart) {
                addToCart({ user: user.id, product: product.id!, quantity: numberProduct })
            }
            else {
                                ...
                                <table>
                                    <tbody>
                                        ...
                                        <tr>
                                            <th>Category</th>
                                            <td>:</td>
                                            <td className="itemss capitalize">{isLoadingCate ? "Loading..." : listCategory.length > 0 && listCategory.map((item: ICategory) => { if (item.id == data.category) return item.name })}</td>
                                        </tr>
                                        {/* <tr>
                                            <th>Tags</th>
                                            <td>:</td>
            ...
            <section className="news">
                <div className="container">
                    ...
                    <div className="section-body">
                        {isLoadingList ? <div>Loading...</div> : <ListProductPage data={listProduct1} />}
                        <div className={`relate-product-showmore ${(!isLoadingList && (listProduct.length <= limit)) && "hidden"}`}>
